...
public class Math3d {
	...
	// Convert spherical coordinates to cartesian. Note: theta and phi do not need to be bounded
	// In: theta:(-pi,pi], phi:[-pi/2,pi/2], radius,
	// Out: x, y, z
	public static Vector3 sphericalToCartasian(float theta, float phi, float radius)
	{
		return new Vector3 (Mathf.Cos (theta) * Mathf.Cos (phi) * radius,
